...
<body>
            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('form.store') }}" method="POST" id="evaluationForm"
                            ...
                            <div class="mb-3">
                                <label for="languages" class="form-label">Languages <span
                                        class="text-danger">*</span></label>
                                <select class="form-control select2 @error('languages') is-invalid @enderror"
                                    id="languages" name="languages[]" multiple="multiple" required>
                                    @foreach ($languages as $language)
                                        <option value="{{ $language->name }}"
                                            {{ in_array($language->name, old('languages', [])) ? 'selected' : '' }}>
                                            {{ $language->name }}</option>
                                    @endforeach
                                </select>
                                <small class="text-muted">You can type to add a new Programming language.</small>
                                @error('languages')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                    rows="3" placeholder="Add a description...">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <div id="issuesContainer">
                                        ...
                                        @foreach (old('issues') as $index => $issue)
                                            <div class="input-group mb-2 issue-row">
                                                <input type="text" name="issues[]"
                                                    ...
                                                    placeholder="Enter an issue" value="{{ $issue }}">
                                                @if ($index > 0)
                                                    <button class="btn btn-outline-danger remove-issue-btn"
                                                        type="button">Remove</button>
                                                @else
                                                    <button class="btn btn-outline-danger remove-issue-btn"
                                                        type="button" style="display:none;">Remove</button>
                                                @endif
                                                @error('issues.' . $index)
                                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                                @enderror
                                    ...
                                    @else
                                        <div class="input-group mb-2 issue-row">
                                            ...
                                            <button class="btn btn-outline-danger remove-issue-btn" type="button"
                                                style="display:none;">Remove</button>
                                        </div>
                                    @endif
                                </div>
                                <button type="button" class="btn btn-outline-secondary btn-sm mt-1"
                                    id="addIssueBtn">+ Add Another Issue</button>
                                @error('issues')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <button type="button" id="showImageBtn"
                                    class="btn btn-outline-primary btn-sm mb-2">+ Add Images</button>
                                ...
                                <div id="imageUploadContainer" style="display: none;">
                                    ...
                                    <div id="imageInputs">
                                        <div class="image-row">
                                            <div class="input-group">
                                                <input
                                                    class="form-control image-file-input @error('images.*') is-invalid @enderror @error('images') is-invalid @enderror"
                                    ...
                                    <button type="button" class="btn btn-secondary btn-sm mt-1"
                                        id="addMoreImagesBtn">+ Add Another Image</button>
                                    <small class="text-muted d-block mt-2">Format: JPG, JPEG, PNG, GIF, WEBP. Max 5
                                        images, up to 5MB each.</small>
                                    @error('images')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                    @error('images.*')
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step="0.1"
                                        class="form-control @error('max_marks') is-invalid @enderror" id="max_marks"
                                        name="max_marks" value="{{ old('max_marks', 10) }}" min="1" required>
                                    @error('max_marks')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.select2').select2({
                tags: true,
                placeholder: "Select or add Programming languages",
                tokenSeparators: [',']
            });
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        if (preview.is('img')) {
                            ...
                            preview.replaceWith(
                                `<img class="img-preview-thumb" src="${e.target.result}" alt="Preview" title="${file.name}">`
                                );
                        }
                    };
